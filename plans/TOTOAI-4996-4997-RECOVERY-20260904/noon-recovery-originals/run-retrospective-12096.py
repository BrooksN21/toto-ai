...
def validate_primary_state(
    path: Path, drawing_id: int, drawing_number: int, package_sha256: str
) -> dict[str, Any] | None:
    if not path.exists():
        return None
    state = read_json(path, "post-draw state")
    verify_self_hash(state, "state_sha256", "post-draw state")
    if state.get("drawing_id") != drawing_id or state.get("drawing_number") != drawing_number:
        raise ValueError("post-draw state identity mismatch")
    if state.get("status") == "complete" and state.get("package_sha256") != package_sha256:
        raise ValueError("post-draw state package SHA-256 mismatch")
    return state
...
def main() -> int:
    ...
    try:
        ...
        primary = validate_primary_state(
            Path(plan["primary_post_draw_state"]),
            plan["drawing_id"],
            plan["drawing_number"],
            plan["issued_operator_package"]["package_sha256"],
        )
