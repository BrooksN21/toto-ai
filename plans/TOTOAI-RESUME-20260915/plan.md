...
## Current step and immediate deliverable
**C1–C2 incomplete; start A now, then B. Missing historical sports data must not block adopting/tests of the already reviewed predictor library or building the honest RAW replay adapter.** C7 skills exist; publication belongs to the separate worker. C6 refactoring is not restarted here. The user has now requested implementation/evaluation; old “planning-only authorization absent” notes are historical, not grounds to wait again.

Input handoff: [model-context.md](/Users/turshevr/toto-ai/plans/TOTOAI-RESUME-20260915/model-context.md). Machine checklist: [machinechecklist.json](/Users/turshevr/toto-ai/plans/TOTOAI-RESUME-20260915/machinechecklist.json). Each stage must update its own result receipt and hand off to parent; parent updates ACTIVE_PLAN after concurrent publication. Never mark an implementation stage complete merely because this plan exists.
...
Three evidence domains stay separate: genuine frozen pre-draw capture; reconstructed historical research with demonstrated historical availability; prospective newly collected data. A late fetch timestamp is retained, never rewritten as early capture. Retrospective data influenced by prior analysis cannot become a blind test. Successful retrospective diagnostics do not bypass the established30drawings/450events,≥70%coverage prospective gate.
...
- 5001–5006 missing market/pool retained; football-history reconstruction does not fabricate them
...
**Why blocked / what is executable now:** 5000 has only three-arm early-market scope;5001–5006 no predraw market/sports inputs; V3 needsD. Complete table with explicit missing statuses can be produced now.
...
**First task: A only.** Task_idTOTOAI-RESUME-20260915. Read this plan sectionA and model-context; verify exact accepted review06 F4 implementation and dependencies from existing isolated worktree as read-only. Adopt only inert probability/features/F4/generation/safety modules and corresponding focused tests into `/Users/turshevr/toto-ai`, preserving current main source and all live import paths. Do not apply whole snapshot04, do not touch final_hybrid_comparison/CLI/scheduler/config/DB/consent/memory/Git while publication runs. If any adopted module requires a new change to an existing live dependency, save the exact missing dependency and stop that integration boundary rather than modifying5007. No fitting/network/replay yet. Return source-file list, accepted-source hash mapping, focused pytest/Ruff outcomes and `A-library-adoption.json`. Independent review follows; parent then assignsB without waiting for missing sports data. Do not start nested workers. No commit/push: separate publication owns it.
