...
from toto_ai.runner.final_input import load_final_input, persist_final_input
...
assert now < datetime(2026, 9, 6, 13, 47, tzinfo=timezone.utc), 'Too late for8min diagnostic before16:55'
raw = load_drawing_detail_cache(12102, cache_dir=M/'data/raw', max_age_seconds=None, allowed_root=M)
...
out = M / 'reports/research/4998' / ('NONFINAL-RESEARCH-DIAGNOSTIC-' + stamp)
...
snapshot = persist_final_input(raw.payload, plan=plan, attempt_id='research-nonfinal-'+stamp, captured_at=raw.fetched_at, destination=out/'input/NONFINAL-plan-bound-input.json', timing_override_sha256=None)
snapshot = load_final_input(snapshot.path, expected_plan=plan)
...
receipt_path = D/'4998_NONFINAL_DIAGNOSTIC_RECEIPT.json'
receipt = {'task_id':'TOTOAI-4998-PARALLEL-INTEGRATION-20260906','status':'VALIDATED_NONFINAL_INPUT_STARTING','classification':'NONFINAL_PRE_DRAW_NONOPERATOR_RESEARCH_DIAGNOSTIC','source_cache':str(raw.path),'source_cache_file_sha256_at_freeze':H(raw.path),'source_payload_sha256':raw.payload_sha256,'source_fetched_at':raw.fetched_at.isoformat(),'input_path':str(snapshot.path),'snapshot_sha256':snapshot.snapshot_sha256,'snapshot_file_sha256':H(snapshot.path),'native_constructor':'persist_final_input native typed envelope also used by morning-training; NOT scheduler final provenance','plan_id':plan.plan_id,'drawing':4998,'drawing_id':12102,'bank':4980,'stake':30,'seed':str(seed),'seed_file_sha256':H(seed),'seed_artifact_sha256':sports.artifact_sha256,'seed_as_of':sports.as_of.isoformat(),'output_dir':str(out),'started_at':now.isoformat(),'outer_timeout_seconds':480,'hard_stop_utc':'2026-09-06T13:55:00Z','protected_before':protected,'source_hashes':source_hashes,'automatic_wagering':False,'operator_compatible':False,'scheduler_final_input':False,'real_api_fetches':0,'db_writes':False,'operator_delivery':False}
...
while process.poll() is None:
    ...
    if elapsed>=480 or datetime.now(timezone.utc)>=datetime(2026,9,6,13,55,tzinfo=timezone.utc):
