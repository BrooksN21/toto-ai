...
def build_v3_predictors(
    *,
    ...
    rejected = False
    # Common entity scope is independent of optional standings league/season.
    # A team's rolling/rest history may legitimately cross competitions/seasons.
    scoped = exact
    ...
    if exact:
        ...
        augment_observed_features(
            features, matches, target, minimum_prior_matches, independent
        )
    for side, blob in (standings or {}).items():
        ...
        _check(
            target["league_id"] is not None
            and target["season"] is not None
            and document["team_id"] == target[side + "_team_id"]
...
def augment_observed_features(features, matches, target, minimum, independent=None):
    """Pure numeric derivation; conveys no identity, review or fit authority."""
    ...
    for side in ("home", "away"):
        _extras(features, matches[side], target, side, independent or {}, minimum)
