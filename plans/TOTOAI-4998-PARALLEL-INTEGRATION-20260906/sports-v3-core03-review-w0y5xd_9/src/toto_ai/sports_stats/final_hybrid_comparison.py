...
def execute_final_hybrid_comparison(
    *,
    ...
    v3_research_request: Mapping[str, Any] | None = None,
    ...
    if v3_research_request is not None:
        # A separate research namespace, after primary delivery/ranking and the
        # unchanged four-strategy selector. Never passed to operator selection.
        from toto_ai.sports_stats.v3_parallel import run_v3_parallel_research

        try:
            raw_events = sorted(
                snapshot.payload["data"]["events"], key=lambda e: e["order"]
            )
            report["v3_probability_research"] = run_v3_parallel_research(
                v3_research_request,
                context={
                    "drawing_id": frozen.drawing_id,
                    "drawing_number": frozen.drawing_number,
                    "drawing_fingerprint": frozen.drawing_fingerprint,
                    "scheduler_plan_sha256": hashlib.sha256(
                        plan_path.read_bytes()
                    ).hexdigest(),
                    "final_input_sha256": snapshot.snapshot_sha256,
                    "captured_at": _timestamp(snapshot.captured_at),
                    "event_ids": [str(e["id"]) for e in raw_events],
                    "bk_probabilities": frozen.bk_probability_matrix,
                    "bk_margins": [None] * 15,
                    "frozen_input": asdict(frozen),
                    "ev_config": asdict(config),
                },
                packages={
                    "quality-v2": baseline.coupons,
                    "sports-shadow": sports_result.coupons,
                    "quality-v3": quality_v3.selected_coupons,
                    "robust": robust_coupons,
                },
                deadline=deadline,
            )
        except Exception as error:
            report["v3_probability_research"] = {
