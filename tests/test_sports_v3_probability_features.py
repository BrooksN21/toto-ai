...
def altered_inputs(*, league=None, identity_conflict=False):
    arguments = inputs()
    target = fixtures.json.loads(arguments["event"].content)
    target.update(league_id=league, season=None)
    documents = {}
    for side, blobs in arguments["verified_event_local_histories"].items():
        doc = fixtures.json.loads(blobs[0].content)
        rows = doc["payload"]["data"]
        for index, row in enumerate(rows):
            row.update(leagueId=index + 300, leagueYear=2018 + index)
            if identity_conflict and side == "home":
                row["gender"] = "conflicting"
        documents[side] = [fixtures._source(side, rows)]
    revised = fixtures._inputs(target, documents)
    revised.pop("cutoff_evidence")
    return {**arguments, **revised}


def test_optional_standings_scope_absence_does_not_block_rolling_core_scope():
    baseline = adapter.build_v3_predictors(**inputs())
    row = adapter.build_v3_predictors(**altered_inputs())
    assert row["scope_verified"] is True
    assert row["source_rejected"] is False
    assert row["features"] == baseline["features"]


def test_removing_optional_league_dependency_keeps_identity_conflict_rejection():
    row = adapter.build_v3_predictors(**altered_inputs(identity_conflict=True))
    assert row["source_rejected"] is True
    assert any("scope conflict" in r for r in row["missing_reasons"])
